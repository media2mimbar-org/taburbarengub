...
**Status penilaian tidak disimpan** (migrasi `20260926010000_status_naskah_turunan.sql`, keputusan Akami 26 Sep). Peserta tidak diberi tahu naskahnya sudah dinilai sebelum raport terbit: tidak ada manfaatnya, dan berlawanan dengan raport serentak. Kolom `writing_submissions.status` dibuang karena terbaca penulis; "sudah dinilai" = ada baris `penilaian_naskah`, yang hanya terbaca penilai. Penjaga "selesaikan" di `ubah_status_kloter` kini mencari naskah mengikat tanpa baris penilaian, dan `nilai_karya` mengembalikan baris penilaiannya. Data ganda (kolom status vs baris penilaian) ikut hilang. Kanal arahan revisi di WA (japri atau grup) dibiarkan terbuka: panel cukup menyediakan tombol chat ke nomor penulis.

Tes: 11 assertion baru di `supabase/tests/baseline.test.sql` (penulis, log baca, penilai tercatat, menilai ulang, penulis tidak membaca nilai). Diuji-mutasi: policy `penilaian_naskah` yang dilonggarkan untuk penulis membuat tes "penulis tidak bisa membaca nilai sebelum raport" merah; penjaga "selesaikan" tanpa cek baris penilaian membuat kloter tertutup dengan naskah belum dinilai dan tes merah.
