...
- **Grading (§3.12)**: grades live in `penilaian_naskah` (graders only; `dinilai_oleh` from the session). `writing_submissions` has no grade or status column: "graded" = a `penilaian_naskah` row exists, and authors must not learn it before the raport. WhatsApp carries revision talk (DM or group, mentor's choice); the web holds everything that counts (files, versions, deadlines, grades).
